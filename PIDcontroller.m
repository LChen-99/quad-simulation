...
L = params.arm_length;
% PID
kp_x = 5; kp_y = 5; kp_z = 5;
kd_x = 1.2; kd_y = 1.8; kd_z = 1.5;
kp_phi = 150; kp_theta = 150; kp_psi = 100;
kd_phi = 30; kd_theta = 30; kd_psi = 30;
...
% Thrust
ddx = des_ddx + kd_x *(des_dx - dx) + kp_x * (des_x - x);
ddy = des_ddy + kd_y *(des_dy - dy) + kp_y * (des_y - y);
ddz = des_ddz + kd_z *(des_dz - dz) + kp_z * (des_z - z);
F = m * (g + ddz);
% 
...
%   des_state.yaw,
%   des_state.yawdot
des_phi = (ddx * sin(psi) - ddy * cos(psi))/ g;
des_theta = (ddx * cos(psi) + ddy * sin(psi))/ g;
